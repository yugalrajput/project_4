...
public class MarksheetModel {
	...
	public void update(MarksheetBean bean) throws ApplicationException, DuplicateRecordException {
		...
		StudentModel sModel = new StudentModel(); // get Student Name
		StudentBean studentbean = sModel.findByPk(bean.getStudentId());
		bean.setName(studentbean.getFirstName() + " " + studentbean.getLastName());
...
//			throw new ApplicationException("Exception in updating Marksheet ");
	...
	public List list(int pageNo, int pageSize) throws ApplicationException {
		...
		if (pageSize > 0) { // if page size is greater than zero then apply pagination

			pageNo = (pageNo - 1) * pageSize; // Calculate start record index
			sql.append(" limit " + pageNo + "," + pageSize);
		}
		Connection conn = null;
		try {
